...
static IRAM_ATTR int line_intersect(const struct quirc_point *p0,
...
static IRAM_ATTR void perspective_setup(float *c,
...
static IRAM_ATTR void perspective_map(const float *c,
...
static IRAM_ATTR void perspective_unmap(const float *c,
...
static IRAM_ATTR void flood_fill_seed(struct quirc *q, int x, int y, int from, int to,
...
static IRAM_ATTR void area_count(void *user_data, int y, int left, int right)
{
	((struct quirc_region *)user_data)->count += right - left + 1;
}

static IRAM_ATTR int region_code(struct quirc *q, int x, int y)
{
	int pixel;
...
static IRAM_ATTR void find_one_corner(void *user_data, int y, int left, int right)
{
...
static IRAM_ATTR void find_other_corners(void *user_data, int y, int left, int right)
{
	struct polygon_score_data *psd =
		(struct polygon_score_data *)user_data;
	int xs[2] = {left, right};
...
static IRAM_ATTR void find_region_corners(struct quirc *q,
...
static IRAM_ATTR void record_capstone(struct quirc *q, int ring, int stone)
{
...
static IRAM_ATTR void test_capstone(struct quirc *q, int x, int y, int *pb)
{
...
static IRAM_ATTR void finder_scan(struct quirc *q, int y)
{
	quirc_pixel_t *row = q->pixels + y * q->w;
	int x;
	int last_color = 0;
...
static IRAM_ATTR void find_alignment_pattern(struct quirc *q, int index)
{
...
static IRAM_ATTR void find_leftmost_to_line(void *user_data, int y, int left, int right)
{
	struct polygon_score_data *psd =
		(struct polygon_score_data *)user_data;
	int xs[2] = {left, right};
 ...
 */
static IRAM_ATTR int timing_scan(const struct quirc *q,
 ...
 */
static IRAM_ATTR int measure_timing_pattern(struct quirc *q, int index)
{
	struct quirc_grid *qr = &q->grids[index];
	int i;
	int scan;
 ...
 * out of image bounds.
 */
static IRAM_ATTR int read_cell(const struct quirc *q, int index, int x, int y)
{
...
static IRAM_ATTR int fitness_cell(const struct quirc *q, int index, int x, int y)
{
	const struct quirc_grid *qr = &q->grids[index];
	int score = 0;
	int u, v;
...
static IRAM_ATTR int fitness_ring(const struct quirc *q, int index, int cx, int cy,
			int radius)
{
	int i;
	int score = 0;
...
static IRAM_ATTR int fitness_apat(const struct quirc *q, int index, int cx, int cy)
{
...
static IRAM_ATTR int fitness_capstone(const struct quirc *q, int index, int x, int y)
{
	x += 3;
	y += 3;
 ...
 * grid.
 */
static IRAM_ATTR int fitness_all(const struct quirc *q, int index)
{
	const struct quirc_grid *qr = &q->grids[index];
...
static IRAM_ATTR void jiggle_perspective(struct quirc *q, int index)
{
	struct quirc_grid *qr = &q->grids[index];
 ...
 * transform.
 */
static IRAM_ATTR void setup_qr_perspective(struct quirc *q, int index)
{
	struct quirc_grid *qr = &q->grids[index];
 ...
 */
static IRAM_ATTR void rotate_capstone(struct quirc_capstone *cap,
...
static IRAM_ATTR void record_qr_grid(struct quirc *q, int a, int b, int c)
{
...
static IRAM_ATTR void test_neighbours(struct quirc *q, int i,
...
static IRAM_ATTR void test_grouping(struct quirc *q, int i)
{
...
static IRAM_ATTR void pixels_setup(struct quirc *q, uint8_t threshold)
{
...
void IRAM_ATTR quirc_end(struct quirc *q)
{
	int i;
...
void IRAM_ATTR quirc_extract(const struct quirc *q, int index,
