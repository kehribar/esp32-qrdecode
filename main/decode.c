...
static IRAM_ATTR void poly_add(uint8_t *dst, const uint8_t *src, uint8_t c,
...
static IRAM_ATTR uint8_t poly_eval(const uint8_t *s, uint8_t x,
...
static IRAM_ATTR void berlekamp_massey(const uint8_t *s, int N,
			     const struct galois_field *gf,
			     uint8_t *sigma)
{
	uint8_t C[MAX_POLY];
...
static IRAM_ATTR int block_syndromes(const uint8_t *data, int bs, int npar, uint8_t *s)
{
	int nonzero = 0;
	int i;
...
static IRAM_ATTR void eloc_poly(uint8_t *omega,
...
static IRAM_ATTR quirc_decode_error_t correct_block(uint8_t *data,
...
static IRAM_ATTR int format_syndromes(uint16_t u, uint8_t *s)
{
	int i;
	int nonzero = 0;
...
static IRAM_ATTR quirc_decode_error_t correct_format(uint16_t *f_ret)
{
	uint16_t u = *f_ret;
	int i;
	uint8_t s[MAX_POLY];
...
static IRAM_ATTR quirc_decode_error_t read_format(const struct quirc_code *code,
...
static IRAM_ATTR int mask_bit(int mask, int i, int j)
{
	switch (mask) {
	case 0: return !((i + j) % 2);
	case 1: return !(i % 2);
...
static IRAM_ATTR int reserved_cell(int version, int i, int j)
{
...
static IRAM_ATTR void read_bit(const struct quirc_code *code,
...
static IRAM_ATTR void read_data(const struct quirc_code *code,
		      struct quirc_data *data,
...
static IRAM_ATTR quirc_decode_error_t codestream_ecc(struct quirc_data *data,
					   struct datastream *ds)
{
...
static IRAM_ATTR inline int bits_remaining(const struct datastream *ds)
{
	return ds->data_bits - ds->ptr;
}

static IRAM_ATTR int take_bits(struct datastream *ds, int len)
{
	int ret = 0;
...
static IRAM_ATTR int numeric_tuple(struct quirc_data *data,
			 struct datastream *ds,
			 int bits, int digits)
{
	int tuple;
...
static IRAM_ATTR quirc_decode_error_t decode_numeric(struct quirc_data *data,
					   struct datastream *ds)
{
	int bits = 14;
	int count;
...
static IRAM_ATTR int alpha_tuple(struct quirc_data *data,
		       struct datastream *ds,
		       int bits, int digits)
{
	int tuple;
...
static IRAM_ATTR quirc_decode_error_t decode_alpha(struct quirc_data *data,
					 struct datastream *ds)
{
	int bits = 13;
	int count;
...
static IRAM_ATTR quirc_decode_error_t decode_byte(struct quirc_data *data,
					struct datastream *ds)
{
	int bits = 16;
	int count;
...
static IRAM_ATTR quirc_decode_error_t decode_kanji(struct quirc_data *data,
					 struct datastream *ds)
{
	int bits = 12;
	int count;
...
static IRAM_ATTR quirc_decode_error_t decode_eci(struct quirc_data *data,
				       struct datastream *ds)
{
...
static IRAM_ATTR quirc_decode_error_t decode_payload(struct quirc_data *data,
					   struct datastream *ds)
{
...
quirc_decode_error_t IRAM_ATTR quirc_decode(const struct quirc_code *code,
				  struct quirc_data *data)
{
	quirc_decode_error_t err;
	struct datastream ds;
